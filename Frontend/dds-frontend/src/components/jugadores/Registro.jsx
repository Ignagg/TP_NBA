...
export default function Registro() {
  ...
  const onSubmit = async (data) => {
    try {
      const res = await backend.crearJugador(data);
      if (res) {
        setAction("C");
  ...
  return (
    ...
    >
      {action === "R" && (
        ...
        >
          <h5 style={{ textAlign: "center" }}>Registro de Nueva Jugador</h5>
          ...
          <div className="form-outline mb-4">
            <label className="form-label" htmlFor="Nombre_del_Jugador">
              Nombre
            </label>
            <input
              type="text"
              id="Nombre_del_Jugador"
              className="form-control"
              {...register("Nombre_del_Jugador", { required: "Este campo es requerido" })}
            />
            {errors.Nombre_del_Jugador && (
              <span className="text-danger">{errors.Nombre_del_Jugador.message}</span>
            )}
          </div>
          <div className="form-outline mb-4">
          ...
          <div className="form-group text-center mt-3">
            <Link to="/consultarJugadores" className="btn btn-dark">
              Volver
            </Link>
          </div>
        </form>
