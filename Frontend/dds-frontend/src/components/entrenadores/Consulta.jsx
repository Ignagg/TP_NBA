...
export default function Consulta() {
    ...
    const loadData = async () => {
        try {
            const res = await backend.obtenerEntrenadores();
    ...
    const handleDeleteUser = async (Id_Entrenador) => {
        await backend.eliminarEntrenadores(Id_Entrenador);
        await loadData();
    };
    ...
    return (
        <div className="imagenFondo">
            <div className="container mt-5">
                ...
                <h2 className="text-left mb-5" id="title" style={{ color: "black" }}>Entrenadores</h2>
                <div className="mb-4">
                    <input type="text"
                    ...
                    <table className="table table-bordered">
                        ...
                        {rows && rows.map((item) => (
                            <tr key={item.Id_Entrenador}>
                                ...
                                <td>{item.Nombre_del_Entrenador}</td>
                                <td>{item.Fecha_de_Nacimiento}</td>
                                <td>
                                    <button
                                        onClick={async () => await handleDeleteUser(item.Id_Entrenador)}
                                        className="btn btn-danger mx-1">
                                        Eliminar
                                    </button>
                                    <button
                                        onClick={() => navigate(`/actualizarEntrenadores/${item.Id_Entrenador}`)}
                ...
                <div style={{textAlign:"center"}}>
                    <Link className='btn btn-dark mx-2 mb-5' to='/RegistrarEntrenadores'>Registrar Entrenador</Link>
